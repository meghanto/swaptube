...
HOST_DEVICE inline float smoother1(float x){return 3*x*x-2*x*x*x;}
HOST_DEVICE inline double smoother1(double x){return 3*x*x-2*x*x*x;}
...
HOST_DEVICE inline double float_lerp(double a, double b, double w){return a*(1-w)+b*w;}
...
HOST_DEVICE inline bool is_single_letter(const std::string& str) {return str.length() == 1 && std::isalpha(static_cast<unsigned char>(str[0]));}
